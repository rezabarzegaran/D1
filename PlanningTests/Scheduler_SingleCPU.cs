        ...
        public class SingleCore
        {
            ...
            public class SingleTask
            {
                ...
                public void Success_EarliestActivation()
                {
                    ...
                    for (int i = 0; i < 50; i++)
                    {
                        ...
                        Simulation s = new Simulation(workload, configuration, 1, 1, 1, 1);
                        Schedule schedule = s.GetSchedule();
                        schedule.Run();

                        Job job = schedule.Tasks[0];
                        List<IGrouping<int, Job.Execution>> trace = Utility.BoundTrace(schedule.Environment, job).GroupBy(x => x.Iteration).ToList();
                ...
                public void Success_Macrotick()
                {
                    ...
                    for (int i = 0; i < 500; i++)
                    {
                        ...
                        Assert.IsTrue(Utility.VerifyMacrotickCompliance(schedule.Environment, trace[0].ToList(), job), $"Macrotick compliance failed for task: {job.Name},0");
                        ...
                        Assert.IsTrue(Utility.VerifyMacrotickCompliance(schedule.Environment, trace[1].ToList(), job), $"Macrotick compliance failed for task: {job.Name},1");
            ...
            public class MultipleTasks
            {
                ...
                public void Success_ConcreteTasks_1()
                {
                    ...
                    // Second task
                    job = schedule.Tasks[1];
                    trace = Utility.BoundTrace(schedule.Environment, job).GroupBy(x => x.Iteration).ToList();
                ...
                public void Success_Macrotick()
                {
                    ...
                    for (int i = 0; i < 500; i++)
                    {
                        ...
                        foreach (Job job in schedule.Tasks)
                        {
                            foreach (var trace in Utility.BoundTrace(schedule.Environment, job).GroupBy(x => x.Iteration))
                            {
                                ...
                                Assert.IsTrue(Utility.VerifyMacrotickCompliance(schedule.Environment, trace.ToList(), job), $"Macrotick compliance failed for task: {job.Name},0");
                            }
                        }
                    }
                }
                ...
                public void Success_TwoTasks_1()
                {
                    ...
                    foreach (Job job in schedule.Tasks)
                    {
                        foreach (var traceFragment in Utility.BoundTrace(schedule.Environment, job).GroupBy(x => x.Iteration))
                        {
